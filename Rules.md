...
| C6 | Column | Column | Quality Grade | Assign Gold/Silver/Bronze grade based on null rate and distinctness. | grade ... |
| C6.1 | Column | Column | Null Rate Calculation | Calculate percentage of nulls in column. | null_rate = nulls / len(df) |
| C6.2 | Column | Column | Distinctness Calculation | Calculate distinct value ratio. | distinctness = distinct / len(df) |
...
| C8 | Column | Column | Numeric Statistics | For numeric columns, output is a summary table at the column level. | summary = df[col].describe() |
...
| C8.7 | Column | Group | Percentiles | Calculate percentiles. | percentiles = df[col].quantile([0.25,0.5,0.75]) |
| C8.8 | Column | Group | Outlier Detection | Detect outliers using IQR or Z-score. | outliers = ... |
| C8.9 | Column | Column | Histogram Bins | Show histogram bins for numeric columns. | hist = df[col].hist() |
| C9 | Column | Column | String Statistics | Compute min/max/avg length, character set, pattern frequency for string columns. | ... |
...
| D4 | Dataset | Dataset | Candidate Key Detection | Find columns (single) and column pairs (composite) that uniquely identify rows. | For each col: check uniqueness & nulls. For each col pair: check combined uniqueness & nulls. uniqueness = df[col].nunique() / len(df); is_pk = (nulls==0) & (uniqueness==100) |
| D4.1 | Dataset | Column | Single-Column Key Analysis | Analyze each column for uniqueness and nulls. | uniqueness = df[[col1,col2]].dropna().drop_duplicates().shape[0] / len(df); is_pk = (no nulls) & (uniqueness==100) |
| D4.2 | Dataset | Group | 2-Column Composite Key Analysis | Analyze all 2 pairs of columns for combined uniqueness | ... |
| D5 | Dataset | Dataset | PII Risk Score | Assess risk of PII by counting columns with PII patterns and aggregating risk. | risk = sum(detect_pii(df[col]) for col in df.columns) |
| D5.1 | Dataset | Column | Count of PII Columns | Count columns matching PII regex patterns. | count = sum(detect_pii(df[col])) > threshold |
| D5.2 | Dataset | Dataset | Aggregated Risk Score | Aggregate risk score based on config thresholds. | score = config["pii_risk_scores"][risk_level] |
...
| C5 | Column | Value | Value Distribution | Show frequency table or histogram of values. | ... |
